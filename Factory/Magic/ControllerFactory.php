...
class ControllerFactory extends BaseFactory
{
	...
	public function make($name = null, array $config = array())
	{
		...
		$className = $this->container->getNamespacePrefix($this->getSection()) . 'Controller\\' . $name;

		if (!class_exists($className, true))
		{
			$className = $this->container->getNamespacePrefix($this->getSection()) . 'Controller\\' . $this->container->inflector->singularize($name);
		}
